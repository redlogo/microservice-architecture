...
public class PaymentController {
    ...
    @GetMapping("/payment/findBySerial/{serial}")
    public CommonResult<Payment> findBySerial(@PathVariable("serial") String serial) {
        Payment res = paymentService.findBySerial(serial);
        if (res != null) {
            log.info("***** " + instanceId + " PaymentController /payment/findBySerial/" + serial + " successful *****");
            log.info("***** returned result: " + res.toString());
            return new CommonResult<>(200, "***** " + instanceId + " PaymentController /payment/findBySerial/" + serial + " successful *****", res);
        } else {
            log.info("***** " + instanceId + " PaymentController /payment/findBySerial/" + serial + " failed *****");
            return new CommonResult<>(500, "***** " + instanceId + " PaymentController /payment/findBySerial/" + serial + " failed *****");
        }
    }

    /**
     * Timeout API for test.
     *
     * @return instanceId
     */
